...
try {
    ...
    // Full Student Data
    $stmt = $conn->prepare("SELECT s.std_id, s.student_id, s.nira, sh.shiftName AS study_mode, sh.shiftName AS shift, s.created_at AS entry_time, s.grad_year, s.grade, s.gender, s.pob, s.mother AS mother_name, s.name AS full_name, s.tell AS phone, s.email, IFNULL(p.tell1, 'N/A') AS emergency_contact_parent, c.cl_name AS class_name, cp.campus AS campus_name, sem.semister_name AS semester, f.name AS faculty, d.name AS department, adr.villages AS address, sch.name AS school_name FROM students s LEFT JOIN parents p ON s.parent_no = p.parent_no LEFT JOIN studet_classes sc ON s.std_id = sc.std_id LEFT JOIN classes c ON sc.cls_no = c.cls_no LEFT JOIN campuses cp ON c.camp_no = cp.camp_no LEFT JOIN departments d ON c.dept_no = d.dept_no LEFT JOIN faculties f ON d.faculty_no = f.faculty_no LEFT JOIN semesters sem ON sc.sem_no = sem.sem_no LEFT JOIN address adr ON s.add_no = adr.add_no LEFT JOIN shifts sh ON s.shift_no = sh.shift_no LEFT JOIN school sch ON s.sch_no = sch.sch_no WHERE s.user_id = ? LIMIT 1");
    $stmt->bind_param("i", $userId); $stmt->execute();
